...
| Benchmark（再現可能な比較結果） | [institutional_benchmark_20260713](reports/institutional_benchmark_20260713.md) |
...
institutional governance方針が定義する上限は **research → validated → shadow → paper** です。
ただし永続的な外部承認seal、end-to-end承認サービス、broker paper stackは現在存在しないため、
registryは昇格を実行せず全モデルを`research`に固定します。legacyマクロ/ML委員は
...
分析時のイベント回避に使用します。最新スナップショットは過去時点の利用可能性を
証明できないため、バックテストの `--events` には使用できません。`--events` は
`recorded_at` に加えて `occurrence_id`、連番 `revision`、`effective_from/to`、
`is_tombstone`、`identity_quality` を持つarchiveだけを受理します。価格データ終端を
as-of上限にし、各バーではその時点のlatest-visible vintageだけを使用します。
Python APIから直接PIT archiveを読む場合も
`load_economic_events_csv(path, as_of=<aware UTC>, require_point_in_time=True)` が必要です。
sourceがstable occurrence IDを提供しない行はheuristic identityとして保存できますが、
run artifactは明示的にpromotion不適格になります。旧 `recorded_at` のみのarchiveへは
追記せずエラーにするため、既存ファイルを証跡として保全したうえで、人間承認済みの
運用移行としてfresh archiveを開始してください (`--no-event-archive` で無効化)。
