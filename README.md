...
ただしend-to-end承認サービスとbroker paper stackは現在存在せず、legacy委員参加状態は
shadow/paperのDiscord助言重みだけです。`--promote-live`とPython APIのlive遷移は無効化されており、
...
監視JSONを1回で更新します。候補の昇格はpaperまでに限定され、live反映は行いません。
