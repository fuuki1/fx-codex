...
# 正規化済みイベントに立てるマーカー。二重backfillを冪等にするために使う。
...
def _news_content_hash(item: Mapping[str, object]) -> str:
    """安定なコンテンツハッシュ。キー順に依存せず同じニュースは同じ値になる。"""
...
def audit_store(path: str | Path) -> AuditReport:
    ...
    for line in _iter_lines(target):
        ...
        if items:
            ...
            for item in items:
                unique.add(_news_content_hash(item))
    return AuditReport(
        path=str(target),
...
def _normalize_event(
    ...
    added = 0
    for item in items:
        digest = _news_content_hash(item)
...
def backfill_store(
    ...
    *,
    ...
    report = BackfillReport(
        ...
        news_items_deduplicated=len(news_index),
    ...
    if target.is_file():
        for line in _iter_lines(target):
            ...
            for digest in refs:
                ...
                if _news_content_hash(restored) != str(digest):
                    tampered += 1
