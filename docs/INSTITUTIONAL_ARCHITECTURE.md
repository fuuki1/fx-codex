...
| COT PIT vertical slice | `fx_intel/cot_pit.py`, `macro.py`, `decision_log.py`, `fx_briefing.py`, `tools/cot_pit_pipeline.py` | **Optional/research-only.** Canonical briefing disables legacy TTL COT and admits an explicitly supplied artifact only after source-specific raw replay and typed as-of loading. Dataset and used-record hashes enter decision context. Local release sidecars are not external attestations; no real corpus, scheduled capture service, accepted licence, backtest integration, deployment or promotion eligibility is evidenced. |
...
`cot_pit.py` is one source-specific specialization of that generic boundary. For the configured CFTC contracts it preserves count-bounded stable pagination, observed row-ID revisions and locally bound release sidecars, then reconstructs the exact domain record set before an observation/release as-of join. Start/end count equality does not provide an atomic upstream snapshot when same-count rows mutate mid-capture, and a CFTC-host URI plus local hash does not authenticate the supplied release bytes or time. This specialization does not expand the generic artifact into a system-wide guarantee.
...
| COT availability | `report_date + 3 days` | Release/first-ingested availability | **B, partially delivered for the optional research path.** Effective availability is bounded by observed row ingestion/validation and versioned local release evidence/declared release time. Holiday schedules can be represented, but actual publication is not independently authenticated and no operating corpus is proven. |
...
- Disabled/unavailable/invalid/incomplete/stale COT evidence excludes only the COT feature and records the typed state. It neither attests the remaining macro inputs nor automatically imposes a whole-system veto.
