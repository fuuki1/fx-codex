...
The sync path writes predictions and prices but never labels them. This command
closes that loop: it reads matured pending predictions, scores each one with the
same canonical scorer the offline pipeline uses, and appends an immutable
outcome. Scoring logic is not duplicated here — the store path and the offline
path share `trade_outcome.score_canonical_outcome`, so one definition of
realized net R applies to both.

```bash
runs/python314-safe-venv/bin/python tools/operational_store_score.py \
  --db logs/fx_operational.sqlite3 \
  --writer-id operational-score
```

The sweep is idempotent and safe to schedule:

- A prediction is scored exactly once. `record_outcome` closes its `pending`
  status in the same transaction, so it leaves the due queue permanently.
- A matured prediction whose price path has not been ingested yet is **deferred,
  not written**. Outcomes are immutable, so recording `evaluation_unavailable`
  for a merely late path would freeze a wrong label forever. `reasons`
  in the JSON report explains every deferral.
- `evaluation_unavailable` is reserved for a path that arrived but yields no net
  label (missing cost provenance) — a terminal, auditable answer.
- The writer lock is exclusive; a concurrent run fails with `WriterBusy` rather
