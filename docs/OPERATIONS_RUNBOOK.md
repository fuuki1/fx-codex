...
| 事象 | 発信元 | 宛先/頻度 | 障害時の扱い |
|---|---|---|---|
| 定期分析（融合+時間足別） | `com.fx-codex.briefing` | 分析Webhook、毎時:10 | 送信失敗はbriefingログと鮮度遅延で検知 |
...
if [ "$CRON_STATUS" -eq 0 ]; then
  echo present > "$AUDIT_ROOT/crontab-read-state.txt"
elif [ "$CRON_STATUS" -eq 1 ] \
  && [ "$(awk 'END { print NR }' "$AUDIT_ROOT/crontab-read-error.txt")" -eq 1 ] \
  && rg -q '^(crontab: )?no crontab for [[:alnum:]_.-]+$' \
    "$AUDIT_ROOT/crontab-read-error.txt"; then
  : > "$AUDIT_ROOT/crontab.before"
  echo absent > "$AUDIT_ROOT/crontab-read-state.txt"
else
  echo 'STOP: crontabを読み取れない。空crontabへ置換してはならない'
  exit 1
fi
shasum -a 256 "$AUDIT_ROOT/crontab.before" > "$AUDIT_ROOT/crontab-before-sha256.txt"
...
for label in com.fx-codex.snapshot com.fx-codex.briefing \
  ...
  if launchctl print "gui/$(id -u)/$label" 2>/dev/null \
    | rg -q -- '--promote-live|trader/|executor\.py'; then
    printf '%s\n' "$label" >> "$AUDIT_ROOT/launchctl-live-flags.txt"
  fi
done
pgrep -fl 'fx_briefing|fx_tf_snapshot|fx_briefing_once|run_exclusive\.py.*fx-(briefing|snapshot)|tv_discord_notify|trader|executor\.py|ib_insync|place[_-]?order' \
...
### 2-3. paper/live安全assertion

現行repositoryに発注機能はない。もし旧`trader`プロセス/コンテナ/別checkoutが1つでも残る場合、
移行前後の両方で次を**実値として**確認する。未設定を安全値とみなさない。

- `TRADING_MODE=paper`
- `ALLOW_LIVE=0`
- `STRATEGY_ENABLED=0`
- legacy kill switchが`ON`/`1`（取得不能・未設定なら不合格）
- 実行中コマンド、cron、plistに`--promote-live`がない

containerの環境を確認するときは対象3キーだけを抽出し、全環境変数を監査ファイルへ出さない。

```bash
if ! command -v docker > "$AUDIT_ROOT/docker-command.txt" 2>&1; then
  echo 'STOP: legacy Docker安全状態を検証できない（docker CLIなし）'
  exit 1
fi
if ! docker ps -a --format '{{.ID}} {{.Names}} {{.Image}} {{.Status}}' \
  > "$AUDIT_ROOT/docker-ps.txt" 2> "$AUDIT_ROOT/docker-ps-error.txt"; then
  echo 'STOP: Docker daemon/container一覧へ接続できずlegacy安全状態を検証できない'
  exit 1
fi
LEGACY_CIDS_FILE="$AUDIT_ROOT/legacy-container-ids.txt"
awk 'tolower($0) ~ /(trader|fx-codex)/ {print $1}' \
  "$AUDIT_ROOT/docker-ps.txt" > "$LEGACY_CIDS_FILE"
: > "$AUDIT_ROOT/docker-safe-env.txt"
while IFS= read -r cid; do
  [ -n "$cid" ] || continue
  SAFE_ENV=$(docker inspect "$cid" \
    --format '{{range .Config.Env}}{{println .}}{{end}}' \
    | rg '^(TRADING_MODE|ALLOW_LIVE|STRATEGY_ENABLED)=' || true)
  printf '%s\n' "$cid" "$SAFE_ENV" >> "$AUDIT_ROOT/docker-safe-env.txt"
  test "$(printf '%s\n' "$SAFE_ENV" | awk -F= '$1=="TRADING_MODE" {print $2}')" = paper
  test "$(printf '%s\n' "$SAFE_ENV" | awk -F= '$1=="ALLOW_LIVE" {print $2}')" = 0
  test "$(printf '%s\n' "$SAFE_ENV" | awk -F= '$1=="STRATEGY_ENABLED" {print $2}')" = 0
done < "$LEGACY_CIDS_FILE"

# repository外/非Dockerの旧executorは環境・kill switchをこの手順から安全に
# 証明できないため、存在だけで移行を停止する。PIDを人間が照合して停止後に再実行する。
if rg -i 'trader|executor\.py|ib_insync|place[_-]?order' "$AUDIT_ROOT/processes.txt"; then
  echo 'STOP: 非Dockerのlegacy execution processを検知'
  exit 1
fi

if rg -- '--promote-live' "$AUDIT_ROOT/processes.txt" "$AUDIT_ROOT/crontab.before" \
  "$AUDIT_ROOT"/launchctl-*.txt; then
  echo 'STOP: live昇格指定を検知'
  exit 1
fi
test ! -s "$AUDIT_ROOT/launchctl-live-flags.txt"
```

legacy stackが見つかった場合は、上の3値とkill switchをstack固有のread-onlyコマンドでも確認し、
その出力を`$AUDIT_ROOT/legacy-kill-switch.txt`へ保存する。値が厳密に`ON`または`1`であることを
`test`し、4条件すべてが成立しなければ移行を中止してlegacy executionを停止する。分析移行を理由に
live設定を変更してはならない。許可する段階は**research → validated → shadow → paper**だけで、
`--promote-live`は常に非ゼロ終了でなければならない。

```bash
if [ -s "$LEGACY_CIDS_FILE" ]; then
  # 直前にstack固有のread-onlyコマンドで正規化した値だけをこのファイルへ保存しておく。
  test -s "$AUDIT_ROOT/legacy-kill-switch.txt"
  KILL_SWITCH=$(tr -d '[:space:]' < "$AUDIT_ROOT/legacy-kill-switch.txt")
  case "$KILL_SWITCH" in
    ON|1) ;;
    *) echo 'STOP: legacy kill switchが安全状態ではない'; exit 1 ;;
  esac
fi
```
...
test -s "$AUDIT_ROOT/crontab-read-state.txt"
shasum -a 256 -c "$AUDIT_ROOT/crontab-before-sha256.txt"
...
for label in com.fx-codex.snapshot com.fx-codex.briefing \
  ...
  if launchctl print "gui/$(id -u)/$label" >/dev/null 2>&1; then
    ...
    if launchctl print "gui/$(id -u)/$label" >/dev/null 2>&1; then
      echo "STOP: bootout後もserviceが残存: $label"
      exit 1
    fi
  fi
done
...
pgrep -fl 'fx_briefing|fx_tf_snapshot|tv_discord_notify|trader|executor\.py' \
...
# 旧journalはhashを持たず、新v2 writer/readerは改竄有無を証明できないため
# 読取・追記しない。service停止中に原本をhash付きで保全し、空のv2 active
# journalから開始する。ファイルを削除したりin-place変換してはならない。
for relative in \
  logs/briefing_tf_prices.jsonl \
  logs/briefing_journal.jsonl \
  logs/briefing_tf_journal.jsonl \
  logs/briefing_decisions.jsonl; do
  if [ -f "$ROOT/$relative" ]; then
    name=$(basename "$relative" .jsonl)
    python3 "$ROOT/tools/prepare_v2_price_journal.py" \
      --root "$ROOT" --relative-path "$relative" --dry-run \
      > "$AUDIT_ROOT/${name}-v2-plan.json"
  fi
done
...
# v2 migrationは全dry-run証跡を独立確認した後だけ適用する。各原本は削除
# されずlogs/legacyへ移動し、hash/行数manifestを残す。
for relative in \
  logs/briefing_tf_prices.jsonl \
  logs/briefing_journal.jsonl \
  logs/briefing_tf_journal.jsonl \
  ...
  if [ -f "$relative" ]; then
    name=$(basename "$relative" .jsonl)
    .venv/bin/python tools/prepare_v2_price_journal.py --root "$ROOT" \
      --relative-path "$relative" --apply --confirm-writers-stopped \
      > "$AUDIT_ROOT/${name}-v2-applied.json"
  fi
done
...
現行repositoryにはhash固定lockがないため、`requirements.lock`を独立レビュー付きで追加するまで
この手順は意図的に停止する。`pyproject.toml`の範囲指定だけで当日最新版を解決したり、旧venvを
承認commitへコピーしたり、`--require-hashes`を外して先へ進めてはならない。

導入後、少なくとも2回のsnapshot周期と次の毎時:10をまたいで再確認する。`lsof`/`pgrep`で
価格writerがsnapshot 1経路、判断writerがbriefing 1経路だけであること、5分間隔の価格追記、
毎時の2ジャーナル追記、freshness report更新、ops通知の到達を確認する。確認結果とログの
...
価格ジャーナルはファイル自体の更新時刻だけでなく、各行の記録時刻、
schema v2の再計算可能なcontent hash、`symbol×timeframe`の全期待キー、
値の繰り返しを監視する。次のどれかはCRITICALであり、予測と売買判断を停止する。

- JSONL破損、schema/hash不一致、naive/future timestamp
- 期待キーの一部だけが古い、または欠落している状態
- mtimeは進んでいるが、同一の有効payloadが設定回数を超えて繰り返される状態
- 価格targetで、設定されたauthoritative `source_time`を証明できない状態

現行のcommunity scanner取得にはproviderが保証するsource timestampがない。
`source_time` をローカル取得時刻で代用せずnullとし、
`source_time_unavailable`をdata-quality flagに残す。この行は保存・障害診断には使えるが、
既定の価格freshness targetはCRITICALとなり、新規判断を停止する。provider起点のPIT時刻を
必要とするモデル昇格証拠にも使えない。

freshness reportとは別に、briefingのstrict price-journal readerは同一5分slot・通貨ペア・
時間足のnatural key重複とファイル順の時刻逆行をhard vetoにする。価格journalの再送は
全provenanceを含むcanonical content hashが一致する場合だけ冪等である。
OHLCが同じでも`run_id`、writer、source clockなどが違う行は同一再送とは扱わない。
経済イベントarchiveも全行の`recorded_at`を非減少に保ち、別occurrenceであっても過去時刻への
追記を拒否する。拒否時は既存ファイルを変更せず、clock/sourceを確認してから再実行する。

`status_fx_services.sh`はcronの読取り状態を成功・厳密な「未登録」・不明に分離する。不明は
CRITICALであり、`fx_briefing_once.sh`や`run_exclusive.py`を含むwrapper writer候補も表示する。
「crontabを読めない」を「競合なし」と解釈してはならない。
...
pgrep -fl 'fx_briefing|fx_tf_snapshot|tv_discord_notify|trader|executor\.py|ib_insync|place[_-]?order' || true
...
**現行repositoryにlive実発注経路は存在しない。** 許可される研究段階は
**research → validated → shadow → paper**までで、`--promote-live`は無効、実注文は出さない。
削除前の`trader/`、別checkout、container、LaunchAgentが実機に残っている可能性はrepositoryの
宣言だけでは否定できないため、移行/rollbackの前後に§2-3を実測する。

「設定が見つからない」「stackへ接続できない」は安全assertionの成功ではない。legacy executionが
存在するなら`TRADING_MODE=paper`、`ALLOW_LIVE=0`、`STRATEGY_ENABLED=0`、kill switch `ON/1`を
すべて確認できなければ停止する。この分析系移行からlive設定を有効化・変更する権限は与えられない。
