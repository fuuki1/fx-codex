...
.venv/bin/python -m pip freeze --all > "$AUDIT_ROOT/pip-freeze.txt" 2>&1
...
for label in com.fx-codex.snapshot com.fx-codex.briefing \
  ...
  launchctl print "gui/$(id -u)/$label" \
    > "$AUDIT_ROOT/launchctl-$label.txt" 2>&1 || true
done
...
mkdir -p "$AUDIT_ROOT/launchagents"
find "$HOME/Library/LaunchAgents" -maxdepth 1 -type f \
  -name 'com.fx-codex*.plist*' -exec cp -p {} "$AUDIT_ROOT/launchagents/" \;
find "$AUDIT_ROOT/launchagents" -type f -exec shasum -a 256 {} \; \
  > "$AUDIT_ROOT/launchagents-sha256.txt"
```

Python/pipの証跡取得に失敗した場合も続行しない。依存が再現できない環境をそのまま
...
docker ps --format '{{.ID}} {{.Names}} {{.Image}}' \
  > "$AUDIT_ROOT/docker-ps.txt" 2>&1 || true
...
if [ -d "$OLD_ROOT/.venv" ]; then
  rsync -a "$OLD_ROOT/.venv/" "$ROOT/.venv/"
fi
