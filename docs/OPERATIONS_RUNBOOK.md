...
融合行には`source_cutoff <= max_feature_available_time <= prediction_time`を記録し、
この来歴を持たない旧形式行は可視化には残すがGBDT学習・昇格から除外する。
GBDT artifactはschema 4と`training_contract=fusion-pit-v1`を必須とし、旧artifactは
