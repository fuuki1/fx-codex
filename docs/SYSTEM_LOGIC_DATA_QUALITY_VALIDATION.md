...
このプロジェクトは、単に「上がる／下がる」を出す予測器ではない。次の3つを分離したFX研究・判断支援基盤である。

1. **分析・判断**: テクニカル、ニュース、経済指標、マクロ、GBDTを統合し、条件が悪ければ見送る。
2. **自己採点・学習**: 過去判断を固定ホライズンやTP/SL先着で採点し、重み・確信度・苦手条件を調整する。
3. **バックテスト・オフラインシミュレーション**: コスト、リスク、次足の仮想成立、OOS、walk-forwardを別系統で検証する。

設計面では、未来情報の混入防止、データ不足時の見送り、モデルの不参加ゲート、shadow-only 制約、データ品質とリスクの拒否権が入っている。本リポジトリの終端は shadow 判断とオフラインシミュレーションである。
...
- シグナルは broker 注文APIへ接続されない。仮想成立と仮想ポートフォリオは、保存済み価格を使う分析専用のオフライン計算である。
- 発注クライアント、注文エンドポイント、ポジション変更、口座リスク変更は永久に対象外である。
...
マクロ委員とML委員は `shadow` 意見を計算・記録するだけで、通常判断の複合スコアには参加しない。保存状態が未知または旧形式でも `shadow` へ fail closed する。
...
- 週末休場中は `closed`、高重要指標の前90分～後120分および中重要指標の前60分～後60分は `standby` とする。
...
過去のTP/SL候補は通常判断に自動適用しない。PIT適格データ、分離した train/tune/calibration/test/lockbox、purge/embargo、全試行台帳、コストストレスが揃うまで counterfactual/shadow に留める。ATRが無い場合はSL/TPも期待Rも正しく計算できないため、欠陥データとして警告する。
...
マクロ／ML委員は `shadow` から遷移しない。次の値は自己相関を間引いた shadow 診断の目安であり、判断参加やパラメータ適用を許可するゲートではない。
...
診断が悪化した場合は shadow 出力を停止または評価不可とする。数値、人間承認、互換フラグのいずれも、本リポジトリでの段階遷移や外部執行を有効にしない。
...
- bid/askは保存できるが、tick順序、仮想成立の遅延、スリッページを完全には再現できない。
- 5分の間に起きた急変を見落とす。
...
- 1.5倍コストで赤字になる戦略は shadow 候補としても不合格にする。

### Step 7: 固定 shadow forward

コードも閾値も固定し、未使用期間で最低30日、できれば複数レジームにまたがって shadow 判断とオフライン仮想成立を収集する。

- バックテストと固定 shadow forward の仮想取引数、期待R、勝率、spread、slippage差を比較する。
- 判断が出なかったケースも記録する。
- stale、API失敗、再起動、重複、欠損を含めた運用品質を測る。
- 価格経路は可能ならDukascopyまたはOANDAのOHLC／bid-askで独立再採点する。

### Step 8: shadow-only 終端ゲート

数値条件を満たしても、本リポジトリでは shadow から先へ進めない。

1. shadow decision: 記録と通知のみ
2. offline simulation: 保存済み価格による仮想成立とコスト採点
3. validated shadow: 固定した未使用期間の診断結果を不変成果物として保存
4. terminal: broker、注文、口座への接続なしで終了

外部執行を示すコード、設定、プロセス、または別checkoutを検出した場合は評価を中止し、リポジトリから操作せずインシデントとして報告する。
...
- リスク率ベースのロット管理
- 固定 shadow forward 30日以上

加えて、設定上は全評価月で月8%目標達成を必須条件にしている。これは研究目標としては理解できるが、統計的な汎用合格基準としては強すぎる。毎月8%を満たすよう最適化すると過学習や過大リスクを誘発しうるため、採否の中心は **期待R、DD、コスト耐性、確率較正、shadow forward再現性** に置き、月次目標は学習特徴やリスク上限へ接続しない別KPIとして扱う。

## 9. 現時点の品質評価

| 項目 | 評価 | 理由 |
|---|---|---|
| システム構造 | 良い | 分析、学習、オフラインシミュレーション、監査が分離され、拒否権とshadow-only境界がある |
...
| 外部執行 | 対象外 | broker接続、注文、ポジション変更、口座リスク変更は実装・検証しない |
...
7. **shadow forward差分を蓄積する**: バックテスト、判断時価格、保存済みbid/askによる仮想成立の差を測る。
...
次のマイルストーンは的中率を上げることではない。まず、新形式のpoint-in-timeデータ、OHLC／bid-ask価格経路、重複のないshadow forwardログを十分に集め、コスト控除後期待Rがrolling OOSと固定shadow forwardの両方で再現するかを確認する。それが確証できるまで、ダッシュボードの数字は学習進捗であって、収益保証や外部執行の根拠ではない。
