...
3. **バックテスト・模擬約定**: コスト、リスク、次足でのオフライン模擬約定、OOS、walk-forward、shadow証拠を別系統で検証する。

設計面では、未来情報の混入防止、データ不足時の見送り、モデルの不参加ゲート、段階昇格、人間承認が入っている。システムは恒久的にanalysis-onlyであり、broker発注経路は存在せず、追加も対象外である。
...
- [`promotion.py`](../fx_intel/promotion.py): マクロ委員とML委員の証拠段階を管理する。`paper` / `live` という旧来の段階名が記録に残る場合も、broker接続や配備先を意味しない。
- [`tools/ai_learning_dashboard`](../tools/ai_learning_dashboard/): ログを表示する読み取り専用UIであり、ダッシュボード自身は売買も学習も実行しない。

### 2.3 バックテスト・オフライン模擬約定系

- [`fx_backtester/`](../fx_backtester/): イベント駆動バックテスト、リスク、コスト、walk-forward、PBO/DSR、Monte Carloを扱う。
- `fx_backtester/execution.py` の「execution」は履歴上のオフライン模擬約定であり、broker APIへ接続しない。
- 旧 `trader/` 発注スタックは削除済みであり、復元・代替実装・別名での再作成は禁止されている。
...
マクロ委員の生重みは0.15、ML委員は0.20である。旧来の `paper` / `live` 証拠ラベルを参加条件として読むコードがあっても、それは分析委員会への参加だけを表し、broker executionを有効にしない。`shadow` 中は意見を計算・記録するだけで、判断には影響しない。参加委員が増えた場合は全重みを正規化する。
...
過去のTP/SL候補がオフライン検証を通り、人間承認された場合だけ別のR倍率を分析設定へ適用できる。ATRが無い場合はSL/TPも期待Rも正しく計算できないため、欠陥データとして警告する。
...
マクロ／ML委員の `shadow` から次の証拠ラベルへの遷移には、自己相関を間引いた後で次がすべて必要である。
...
後段の証拠ラベルで40件以上かつ的中率47%未満になるとshadowへ自動降格する。`live` への遷移は証拠や人間承認の有無にかかわらず、このrepositoryでは拒否される。
...
- bid/askは保存できるが、tick順序、模擬遅延、スリッページを完全には再現できない。
- 5分の間に起きた急変を見落とす。
...
- 1.5倍コストで赤字になる戦略は研究候補から除外する。

### Step 7: shadow／forward

コードも閾値も固定し、未使用期間で最低30日、できれば複数レジームにまたがって収集する。

- バックテストとshadowでのオフライン模擬取引数、期待R、勝率、spread、slippage差を比較する。
- 判断が出なかったケースも記録する。
- stale、API失敗、再起動、重複、欠損を含めた運用品質を測る。
- 価格経路は可能ならDukascopyまたはOANDAのOHLC／bid-askで独立再採点する。

### Step 8: research → validated → shadow

数値条件を満たしても、broker executionへは進まない。

1. research: 仮説と履歴検証
2. validated: lockboxを含む独立検証済み
3. shadow: 判断とオフライン模擬約定の記録のみ

データ欠損、重複、監視欠落、再生不一致が一度でも起きたらfail-closedで停止し、前段へ戻す。shadowとオフライン模擬約定がこのrepositoryの終端である。
...
| 項目 | 評価 | 理由 |
|---|---|---|
| システム構造 | 良い | 分析、学習、オフライン模擬約定、監査が分離され、拒否権と段階昇格がある |
...
| broker execution | 対象外 | 発注クライアントとorder endpointは存在せず、追加・復元も禁止 |
...
7. **shadow差分を蓄積する**: バックテスト、判断時価格、オフライン模擬約定、観測した外部quoteの差を測る。
...
次のマイルストーンは的中率を上げることではない。まず、新形式のpoint-in-timeデータ、OHLC／bid-ask価格経路、重複のないforwardログを十分に集め、コスト控除後期待Rがrolling OOSとshadowの両方で再現するかを確認することである。それが確認できるまでは、ダッシュボードの数字は学習進捗であって、収益保証ではない。確認後もbroker executionは対象外である。
