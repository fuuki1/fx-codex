...
**現行システムは分析→Discord通知専用で、注文を出しません。** 旧`trader/`執行stack、
`params_gate.py`、`auto_optimize.py`、`promote_params.py`、`strategy_params.json`は
2026-07-10に削除されました。再作成・live有効化・削除済み文書への依存を行わないでください。
...
- synthetic、research、validated、shadow、paper、liveを混同しない。現行legacy委員は
  非影響shadow固定で、`--promote-live`は無効である。
