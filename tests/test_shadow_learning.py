...
def test_neutral_final_decision_matures_shadow_with_canonical_net_r() -> None:
    ...
    # realized_net_r はbid/ask正準ラベルPR(trade_outcome拡張)で非Noneになる。
    # 本PR時点では執行コスト未保存の判断はNone(キーの存在=スキーマ配線を検証)。
    assert "realized_net_r" in outcome
...
def test_shadow_promotion_does_not_mix_producer_versions_and_resets_stage() -> None:
    ...
    # research buildは昇格機構ごとshadow固定(promotion.update_stages)。
    # producer版が変わってもpaperへ滲まないこと自体は stage=="shadow" で担保される。
    assert any(row["to"] == "shadow" for row in state.history)
