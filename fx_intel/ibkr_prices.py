...
class IbkrPriceConfig:
    ...
    def from_env(cls, *, project_root: str | Path | None = None) -> "IbkrPriceConfig":
        ...
        if port == 4001:
            raise ValueError("IBKR live port 4001 は学習データ取得に使用できません。paper 4002を指定してください")
        if not 1 <= port <= 65535:
    ...
    try:
        ...
        for raw_symbol in symbols:
            ...
            try:
                ...
                warnings.append(
                    f"IBKR paper {symbol} bid/ask OHLC取得失敗: {_error_text(error)}"
                )
...
def _connect(
    ...
    *,
    ...
    if ib_factory is None:
        ...
        ib_factory = IB
        forex = Forex
        fetch_fields: Any = StartupFetch(0)
    else:
        forex = lambda pair: _SimpleForex(pair)  # noqa: E731 - small injectable contract
        fetch_fields = 0
    ib = ib_factory()
    ib.connect(
        config.host,
