...
def _same_aware_instant(value: object, expected: datetime) -> bool:
...
class MacroSnapshot:
    ...
    cot_evidence: dict[str, object] | None = None
    ...
    def fresh_cot(self, currency: str) -> CotReport | None:
        ...
        if (
            ...
            or found is None
            or found.report_date > self.fetched_at.date()
            or found.is_stale(self.fetched_at)
        ):
            return None
        available = found.available_time
        if available is None or available.tzinfo is None:
            return None
        if available.astimezone(UTC) > self.fetched_at.astimezone(UTC):
            return None
        if "publication_time_attested_locally" not in found.data_quality_flags:
            return None
        if not found.source_record_id.strip():
            return None
        if not _is_sha256(found.content_hash) or not _is_sha256(found.dataset_id):
            return None
        evidence = self.cot_evidence
        if (
            not isinstance(evidence, Mapping)
            or evidence.get("status") != "ok"
            or evidence.get("usable") is not True
            or evidence.get("dataset_id") != found.dataset_id
            or not _same_aware_instant(evidence.get("prediction_time"), self.fetched_at)
        ):
            return None
        record_hashes = evidence.get("record_hashes")
        if not isinstance(record_hashes, (list, tuple)) or any(
            not _is_sha256(value) for value in record_hashes
        ):
            return None
        if len(set(record_hashes)) < 2 or found.content_hash not in record_hashes:
            return None
        return found
