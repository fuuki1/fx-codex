...
1. 自己相関の間引き — ブリーフィングは毎時実行され、24時間ホライズンの
   評価窓は隣接エントリと23時間重複する。ほぼ同じ判断を24回数えると
   実効サンプル数を過大評価するため、ペアごとに最低4時間空けて間引く
2. 時系列分割+エンバーゴ — 検証セットは常に学習セットより未来。さらに
   境界の72時間(評価ホライズン+週末)を学習側から落とし、
   学習サンプルの評価窓が検証期間に食い込むリーク(情報漏れ)を防ぐ
3. スキルゲート — 検証セットのBrier/loglossが「基準率をそのまま出す
   無能な予測」を下回らないモデルは usable=False として保存され、
   committee はそれを判断に使わない。学習できた事実と使ってよい事実を
   分離する
4. 較正 — 検証マージンでPlattスケーリングを当て、出力確率が実際の
   的中率と一致するよう補正する(確信の誇張防止)
...
def thin_calls(
    ...
    毎時実行×24時間ホライズンでは隣接判断の評価窓がほぼ重複しており、
...
def train_artifact(
    ...
    if brier_gain >= MIN_BRIER_IMPROVEMENT and logloss_better:
