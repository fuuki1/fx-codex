...
- **金融MLプリミティブ** (`labeling.py`): `indicators.py` と同じ純粋関数層で、López de Prado の3ツールを依存追加ゼロ（numpy/pandasのみ）で提供。`frac_diff_ffd`（分数次差分・固定幅窓＝定常化と記憶保持の両立。d=0で原系列・d=1で1次差分に一致）、`triple_barrier_labels`（上下＋垂直バリアのパス依存ラベリング。`side`を渡すとショート方向にも対応）、`meta_labels`／`sample_weights_by_return`（利益絶対値でのサンプル重み付け＝「精度でなく利益に整合」）。`ai_meta_labeled.py` から合成して使う。**FFDは閾値が小さいほど窓が数百〜数千本に膨らむため、`frac_diff_threshold`（既定1e-3で窓≈50本）で実用長に収める設計**。
...
- **依存の据え置き**: 新モジュール(analyst/macro/dukascopy/gbm/ml/committee/promotion/timeframe/price_history/tf_learning/tf_briefing)は追加のサードパーティ依存を一切増やさない。macro.py・dukascopy.pyの取得はrequestsのみ(dukascopyの展開・パースは標準ライブラリ`lzma`/`struct`)、GBDT・学習・昇格・分析エンジン・時間足別レイヤは標準ライブラリだけで動く。Mac miniの軽量venvへrsyncするだけで移設できる。
- テスト(`tests/test_fx_intel.py`ほか`test_analyst`/`test_macro`/`test_dukascopy`/`test_gbm`/`test_ml`/`test_committee`/`test_promotion`、時間足別は`test_timeframe`/`test_price_history`/`test_tf_learning`/`test_tf_snapshot`/`test_tf_briefing`/`test_fx_briefing_per_timeframe`/`test_dashboard_timeframe`)はネットワーク不要で完結する設計（マクロ取得はキャッシュ事前投入でオフライン検証、Dukascopyは`lzma`+`struct`で合成した`.bi5`とモックセッションで検証、将来価格取得は注入モックで検証）。
