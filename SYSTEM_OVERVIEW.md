...
- 実装済み戦略:
  ...
  - `ai_meta_labeled.py` — **López de Prado 方法論（メタラベリング）戦略**。レポート（`FX AI.md`）ギャップ②の実装。①一次モデル（MAクロス）が方向を出し、②`labeling.py` の**トリプルバリア**（上=利確・下=損切・垂直=時間切れをパス依存で判定）で「その方向が利確に届いたか」をメタラベル化し、③二次モデルが**分数次差分（FFD）で定常化しつつ記憶を残した特徴量**で「張るべきか見送るべきか（=サイズ）」を学習。二次確率が `meta_threshold` 以上のときだけ一次方向にエントリする。メタラベルは`touch_ts`（バリア到達）時点で初めて確定するため、学習は「判断時点までに成熟済みのイベント」だけに限定してリークを防ぐ。**二次モデルは `secondary_model` で切替可能**（既定`"logistic"`=依存ゼロの自前ロジスティック / `"gbdt"`=レポートが実務的王者とする勾配ブースティング木＝`fx_intel.gbm`。学習ループはfit→predict closureでモデル非依存に抽象化。GBDTは非線形・特徴量交互作用を拾える）。CUSUMイベントサンプリング（`use_cusum_events`）も併用可。実データ（Dukascopy）でCLI疎通確認済み。
  - `baselines.py` — buy&hold等、比較対象のベースライン
- **フラクショナル・ケリー＋別枠VaR** (`kelly.py` + `risk.py`拡張): レポート（`FX AI.md`）ギャップ⑧「クォーター〜ハーフケリー／VaR別枠」の実装。`kelly.py`は標準ライブラリのみの純粋関数で、①実現R倍数からケリー比率 `f*=(bp−q)/b` を推定、②`fraction`（既定0.25＝クォーター）を掛けて1トレードリスク%を決める。レポートの2大戒めをコードの既定に落とし込む: **フルケリーは危険**（既定クォーター、`kelly_max_risk_pct`で上限クリップ、決して1.0超にしない）、**標本が貯まるまで使わない**（`kelly_min_trades`=50未満は固定フラクショナルへフォールバック、`full_confidence_trades`=100まで線形ブレンド）。`RiskManager`はトレード確定ごとに`update_risk_budget`でリスク%を更新し、`position_size`に反映する。**既定OFF（`use_fractional_kelly=False`）なので従来の固定1%運用は完全に不変**（後方互換）。VaRはレポートの核心「方向予測モデルはVaR推定に使えない（29.1%違反）」に従い、**サイジングとは独立に実現equityリターン分布そのものから計算する別枠の監視指標**（historical/parametric、`_inv_norm_cdf`はscipy非依存のAcklam近似）。`var_limit_pct`を超えると`can_open`が新規建てを止める。CLIから`--use-fractional-kelly --kelly-fraction 0.5 --var-limit 0.05`等で操作可、実データ（Dukascopy）でCLI疎通確認済み。
- **金融MLプリミティブ** (`labeling.py`): `indicators.py` と同じ純粋関数層で、López de Prado の4ツールを依存追加ゼロ（numpy/pandasのみ）で提供。`frac_diff_ffd`（分数次差分・固定幅窓＝定常化と記憶保持の両立。d=0で原系列・d=1で1次差分に一致）、`triple_barrier_labels`（上下＋垂直バリアのパス依存ラベリング。`side`を渡すとショート方向にも対応）、`meta_labels`／`sample_weights_by_return`（利益絶対値でのサンプル重み付け＝「精度でなく利益に整合」）、`cusum_filter`（**CUSUMイベントサンプリング**＝累積変化が閾値を超えた「意味のある変化点」だけを抽出し、全バー学習の自己相関を避ける。閾値はfloatまたはボラ連動Series）。`ai_meta_labeled.py` から合成して使う（`use_cusum_events=True`でCUSUMサンプリングをON）。**FFDは閾値が小さいほど窓が数百〜数千本に膨らむため、`frac_diff_threshold`（既定1e-3で窓≈50本）で実用長に収める設計**。
- **約定モデル**: シグナルは足確定後にのみ発生、成行は次足始値で約定、買いはAsk/売りはBidという現実的な前提。スプレッド・スリッページが0以下だとバックテスト自体を拒否する。
- **過学習対策**: walk-forwardのパラメータ組み合わせ数を`--max-params`（既定20）で制限、学習区間で選んだパラメータを隣接する未使用テスト区間にのみ適用、勝率単体ではなくSharpe/期待R/PF/DDを組み合わせたスコアで評価。さらに`trial_log.py`（探索全試行の記録）と`overfitting.py`（**PBO/CSCV・Deflated Sharpe Ratio・SPA検定**の統計検定、scipy非依存）を備え、`WalkForwardValidator`にも`trial_logger=`で試行記録を差し込める。`superior_predictive_ability`はHansen(2005)のSPA検定を定常ブートストラップで実装し、「多数戦略の中の最良の優位がデータマイニングの産物か真の予測力か」をp値で判定する（レポートが引用するWhite's Reality Check系の検定）。
- `validation.py`/`qa.py`にデータ品質検証（OHLC整合性、時系列順、重複、spread/slippage設定の妥当性）があり、失敗時はバックテスト自体を実行しない「商用運用ゲート」を構成している。
- **検証パイプライン＝デプロイ合否ゲート** (`validation_pipeline.py` + CLI `deploy-gate`): レポート（`FX AI.md`）の核心「予測精度そのものより総合システム（予測×執行×リスク×検証×規律）で差がつく」を、既にある部品を**1本の判定パイプライン**に結線して体現する層。walk-forward（`WalkForwardValidator`、purge+embargo）→ 全試行を`TrialLogger`で記録 → `overfitting.py`の**PBO(CSCV)・コスト込みDSR・SPA検定** → OOSリターンの勝率系列を`drift.py`の**ADWIN**で監視、を束ねて**デプロイ合否**を判定する（新しい戦略ロジックは足さない結線層）。レポートが「第1段階の閾値」「方針を変えるベンチマーク」として挙げる基準を、そのまま**4つのデプロイ拒否条件**にする: ①コスト込みDSR<0.95（Sharpeが有意でない）、②PBO≥0.5（IS順位にOOS予測力なし）、③SPA p値≥0.05（最良の優位が有意でない）、④OOS期間でADWINがドリフト検出（要再学習=デプロイ保留）。**いずれか1つでも該当すればデプロイ拒否**し、判定理由を必ず文字列で残す（`params_gate`/`promotion`と同じ「来歴+明示基準」思想）。CLIは`deploy-gate`サブコマンドで、拒否時は**終了コード1**を返しCI/デプロイスクリプトのゲートにそのまま使える。合成データ検証では、単純MAクロスがトレンドで方向エッジ（SPA有意）を持っていてもDSR/PBOが通らず拒否される＝「予測精度≠収益」「過小検証な戦略はデプロイしない」というレポートの規律をコードで強制する。
...
- **実ティックデータ層** (`fx_intel/dukascopy.py` + ルートの `fetch_dukascopy.py`): レポート(`FX AI.md`)が個人開発者の実質標準として名指しする **Dukascopy の無料ティックフィード**(ECN流動性プール、bid/ask込み)を、**追加のサードパーティ依存ゼロ**(標準ライブラリ `lzma`/`struct` + requests のみ。dukascopy-node の Node 依存を持ち込まない)で取り込む。`.bi5`(LZMA圧縮された固定長 `>IIIff` レコード列=ms_offset/ask_pts/bid_pts/vol)を1時間ずつ取得し、生バイナリを `runs/dukascopy_cache/` へ保存(Dukascopy の IP 制限対策で二度取らない)。パース(`parse_bi5`)・バー集約(`ticks_to_bars`、mid価格・左閉右開・足なしは捏造しない)・CSV変換(`bars_to_csv_rows`)はすべて**ネットワーク非依存の純粋関数**で、fixtureのバイト列からテストできる。出力CSVは `fx_backtester` のデータ品質ゲートを通る形式(`timestamp,symbol,open,high,low,close,volume,spread_price`)で、**spread_price に Dukascopy の実bid/askスプレッド**が入るため、約定コストを合成値でなく実測値でモデル化できる(契約テスト `test_download_bars_csv_output_loads_in_backtester` が `load_price_csv` を通ることを保証)。さらに `make_future_price_provider()` が **`price_history.py` の源B(`FuturePriceProvider`)を実装**し、時間足別採点で後続ジャーナル行が無い区間の将来価格を確定ティックで補える(源Bは失敗しても静かに `None` を返し採点を止めない)。用途は「実データCSV→`fx_backtester --data`/`auto_optimize.py --data`(合成データゲートを実データで通す)」と「時間足別採点の穴埋め」の2つ。
...
- **ドリフト検出** (`fx_intel/drift.py`): **ADWIN（ADaptive WINdowing、Bifet & Gavaldà 2007）**を標準ライブラリのみで実装（gbm/ml/promotionと同じ依存ゼロ方針）。予測誤差や的中フラグの系列を逐次監視し、可変長ウィンドウをあらゆる位置で古い部分と新しい部分に分割してHoeffding由来の閾値で平均差を検定、「レジームが変わった」瞬間を検出する。変化が無ければ窓は伸び続け（長い窓で安定推定）、あれば古い側を捨てて縮む（直近に即応）。`scan_for_drift`はバッチ走査でドリフト点を列挙し「最後のドリフト以降のデータで再学習」といった再学習トリガーに使える。レポートが挙げるドリフト監視（champion/challengerの再学習判断）の中核。
...
- **依存の据え置き**: 新モジュール(analyst/macro/dukascopy/gbm/ml/committee/promotion/drift/timeframe/price_history/tf_learning/tf_briefing)は追加のサードパーティ依存を一切増やさない。macro.py・dukascopy.pyの取得はrequestsのみ(dukascopyの展開・パースは標準ライブラリ`lzma`/`struct`)、GBDT・学習・昇格・分析エンジン・ADWINドリフト検出・時間足別レイヤは標準ライブラリだけで動く。Mac miniの軽量venvへrsyncするだけで移設できる。
- テスト(`tests/test_fx_intel.py`ほか`test_analyst`/`test_macro`/`test_dukascopy`/`test_gbm`/`test_ml`/`test_committee`/`test_promotion`/`test_drift`、時間足別は`test_timeframe`/`test_price_history`/`test_tf_learning`/`test_tf_snapshot`/`test_tf_briefing`/`test_fx_briefing_per_timeframe`/`test_dashboard_timeframe`)はネットワーク不要で完結する設計（マクロ取得はキャッシュ事前投入でオフライン検証、Dukascopyは`lzma`+`struct`で合成した`.bi5`とモックセッションで検証、将来価格取得は注入モックで検証、ADWINは既知の定常/変化系列で検証）。
