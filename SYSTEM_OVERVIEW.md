...
> **方針（2026-07-10〜）**: 自動売買（発注）はまだ時期尚早と判断し、いったん取りやめました。
...
> 将来また発注を組み込む場合は、この分析基盤の上に別途構築します。

## この文書での「設計」と「観測」

- **正規設計**: Mac mini (`/Users/fuuki/srv/fx-codex`) では、launchdの
  `snapshot`（5分）、`briefing`（毎時:10）、`health`（5分）だけを定期実行する。
...
│  launchd briefing 毎時:10 ─▶ 判断/学習/定期通知   │
...
- **分析・通知** (`fx_intel/` ＋ ルート直下スクリプト): ニュース×経済指標×テクニカル×マクロを統合し、
  複数AI委員会で複合スコアを作り、正規運用では**毎時のブリーフィング**をDiscordへ送る。
...
### 2-1. `tv_discord_notify.py` — シンプル版

- TradingViewスキャナー(`tradingview-ta`)から複数時間足の総合レーティングとRSI/MACD/SMAを取得。
- MAクロス目線（MA 20/100 固定）と突き合わせて一致/不一致を判定。
- Discord Webhookへ送信（`fx_backtester`非依存、単体実行可能）。
- MA窓は保守的な固定値（20/100）を使う。発注戦略の最適化パラメータは持たない。

### 2-2. `fx_briefing.py` + `fx_intel/` — 統合版（上位互換）
...
- `tv_notify_loop.sh` は廃止済み（Discordへ送らず終了する）。
...
| 情報源 | テクニカルのみ | テクニカル＋ニュース＋経済指標＋マクロ |
...
- 自動売買・発注機能は本システムには含まれない。将来組み込む場合は、この分析基盤とは
  別のコンポーネントとして、実発注前の多段ゲートと人間の明示承認を伴う形で設計する。
