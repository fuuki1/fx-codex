...
- 実装済み戦略:
  ...
  - `ai_meta_labeled.py` — **López de Prado 方法論（メタラベリング）戦略**。レポート（`FX AI.md`）ギャップ②の実装。①一次モデル（MAクロス）が方向を出し、②`labeling.py` の**トリプルバリア**（上=利確・下=損切・垂直=時間切れをパス依存で判定）で「その方向が利確に届いたか」をメタラベル化し、③二次ロジスティックが**分数次差分（FFD）で定常化しつつ記憶を残した特徴量**で「張るべきか見送るべきか（=サイズ）」を学習。二次確率が `meta_threshold` 以上のときだけ一次方向にエントリする。メタラベルは`touch_ts`（バリア到達）時点で初めて確定するため、学習は「判断時点までに成熟済みのイベント」だけに限定してリークを防ぐ。実データ（Dukascopy）でCLI疎通確認済み。
