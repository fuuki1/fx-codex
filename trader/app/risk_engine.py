...
# RiskState.now が受け付ける時刻表現（datetime / epoch 秒 / None=現在時刻）
TimeLike = datetime | float | int | None
...
class RiskState:
    ...
    now: TimeLike = None                  # datetime / epoch 秒（within_session 用、None で現在時刻）
...
def in_thin_liquidity(now: TimeLike, windows: list[tuple[int, int]]) -> bool:
...
def evaluate(sig: dict, state: RiskState, params: RiskParams) -> RiskDecision:
    ...
    stop_distance = float(sd_raw) if sd_raw is not None and sd_raw != "" else None
    tp_raw = sig.get("tp_distance")
    tp_distance = float(tp_raw) if tp_raw is not None and tp_raw != "" else None
    ...
    # 3) 取引セッション（時間外は新規しない）
    if params.enforce_session and not within_session(asset, symbol, _as_datetime(state.now)):
...
def _decide_size(
    *, req_qty: float, stop_distance: float | None, factor: float, vpp: float, params: RiskParams
    ...
    if stop_distance is not None and stop_distance > 0:
        return sized, sized * stop_distance * vpp
    return sized, 0.0
...
def _as_timestamp(now: TimeLike) -> float:
...
def _as_datetime(now: TimeLike) -> datetime | None:
    """datetime / epoch / None を datetime へ。None はそのまま（=呼び先で現在時刻扱い）。"""
    if now is None or isinstance(now, datetime):
        return now
    return datetime.fromtimestamp(float(now), tz=UTC)


def _minute_of_day_utc(now: TimeLike) -> int:
