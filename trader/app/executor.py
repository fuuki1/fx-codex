...
- 残存ストップ取消: 同一シンボルへの新規発注（決済・反転含む）前に、前回張った
  保護ストップを取り消す。フラット後に逆指値だけが残って約定すると「意図しない
  新規ポジション」になるため（追跡は Redis の exec:stop_orders ハッシュ）。
...
_PNL_SENTINEL = 1e300
# 保護ストップの追跡（symbol -> {"ref": stop の orderRef}）。反転・決済時の取消に使う。
...
def _track_stop(symbol: str, stop_ref: str | None) -> None:
    """今回の発注で張った保護ストップを記録する（張らなかったなら追跡をクリア）。

    Redis 不通でも発注自体は成立させる（best-effort。次回の取消が空振りするだけで、
    その場合も openTrades 照合により誤取消は起きない）。
    """
    try:
        if stop_ref:
            common.r().hset(KEY_STOP_ORDERS, symbol, json.dumps({"ref": stop_ref}))
        else:
            common.r().hdel(KEY_STOP_ORDERS, symbol)
    except Exception:
        log.exception("failed to track protective stop", **log_extra(symbol=symbol))


def _cancel_tracked_stop(symbol: str) -> None:
    """前回張った保護ストップを取り消す（反転・決済で古い逆指値が残らないように）。

    フラット化後に旧ストップだけが約定すると「意図しない新規ポジション」になる。
    既にストップ到達でクローズ済みなら openTrades() に現れないため無害。
    失敗しても新しい発注は止めない（Discord 警告で手動確認に回す）。
    """
    try:
        raw = cast(str | None, common.r().hget(KEY_STOP_ORDERS, symbol))
        common.r().hdel(KEY_STOP_ORDERS, symbol)
    except Exception:
        log.exception("stop tracking read failed -> skip cancel", **log_extra(symbol=symbol))
        return
    if not raw:
        return
    try:
        ref = str(json.loads(raw).get("ref", ""))
    except Exception:
        return
    if not ref:
        return
    try:
        for t in _ib.openTrades():
            if str(getattr(t.order, "orderRef", "")) == ref:
                _ib.cancelOrder(t.order)
                log.info("cancelled stale protective stop", **log_extra(symbol=symbol, ref=ref))
    except Exception:
        log.exception("failed to cancel existing protective stop", **log_extra(symbol=symbol))
        common.notify(
            f"⚠️ 旧ストップの取消に失敗: {symbol} ref={ref}。IB 上に逆指値が残って"
            f"いないか手動確認してください。",
            key=f"stop_cancel:{symbol}",
        )
...
def handle(sig: dict[str, Any]) -> None:
    ...
    # 反転・決済で旧ストップが残らないよう、同一シンボルの追跡済みストップを先に取り消す
    _cancel_tracked_stop(str(sig["symbol"]))
    ...
    try:
        ...
        _track_stop(str(sig["symbol"]), stop_ref)  # 次回の反転・決済時に取り消せるよう記録
        common.db_execute(
