...
★このファイルはリポジトリ直下 params_gate.py のミラーです（Dockerイメージへ同梱する
  ため app/ 配下にも置く）。片方だけ編集すると生成側と読み込み側の判定がずれて事故に
  なるので、必ず両方を同一内容に保つこと（同期は tests/test_params_gate_sync.py が検証）。
  依存は標準ライブラリのみ（コンテナに追加依存を持ち込まないため）。
