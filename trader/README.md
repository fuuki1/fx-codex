...
```
- **チャート表示**: TradingView 製の無料ライブラリ (Lightweight Charts) で、ローソク＋短期/長期 MA＋
  売買マーカー＋損切り/利確ラインを描画（5 秒ごと自動更新）。
- **助言ロジック**: 上位足(既定 H1)と下位足(既定 M5)が同方向のときだけ「入る」。直近クロスで
  **好機(strong)**、継続中は **セットアップ(押し目/戻り待ち)**。不一致・セッション外・データ不足は
  **WAIT**。損切りは ATR×倍率、利確は R:R 目標（既定 1.5R）。設定は `.env` の `ANALYZER_*`。
